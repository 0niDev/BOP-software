...
class InvoiceSaveThread(QThread):
    ...
    def run(self):
        try:
            if self.is_create:
                success, error = self.controller.create_sales_invoice(**self.kwargs)
                invoice = None
                if success and self.kwargs.get("invoice_number"):
                    # Fetch the created invoice to emit
                    invoices, _ = self.controller.list_sales_invoices()
                    invoice = next(
                ...
                success, error = self.controller.update_sales_invoice(**self.kwargs)
                invoice = None
