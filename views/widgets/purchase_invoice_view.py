"""
Purchase Invoice management widget - with Supplier Search.
"""
...
class InvoiceLoadThread(QThread):
    ...
    def run(self):
        try:
            invoices, error = self.controller.list_purchase_invoices(status=self.status)
...
class SupplierLoadThread(QThread):
    """Background thread for loading suppliers."""
    
    data_loaded = Signal(list, str)  # suppliers, error
    ...
    def run(self):
        try:
            ...
            logger.exception(f"Error in supplier load thread: {e}")
            self.data_loaded.emit([], str(e))
...
class PurchaseItemSelectionDialog(QDialog):
    ...
    def _setup_ui(self) -> None:
        ...
        self.unit_cost_spin.setMinimum(0.0)
        self.unit_cost_spin.setMaximum(999999.99)
        self.unit_cost_spin.setValue(0.0)
        self.unit_cost_spin.setDecimals(2)
        pricing_layout.addRow("Unit Cost*:", self.unit_cost_spin)
        ...
        self.unit_cost_spin.valueChanged.connect(self._calculate_line_total)
        ...
        if item_id:
            ...
            if error:
                ...
                if hasattr(item, 'purchase_price') and item.purchase_price > 0:
                    self.unit_cost_spin.setValue(item.purchase_price)
    ...
    def get_item_data(self) -> dict | None:
        ...
        return {
            ...
            "line_total": (self.quantity_spin.value() * self.unit_cost_spin.value()) 
                         - self.discount_spin.value() + self.tax_spin.value()
        }


class PurchaseInvoiceView(QWidget):
    """Widget for managing purchase invoices with supplier search."""
    
    invoice_created = Signal(PurchaseInvoice)
    invoice_updated = Signal(PurchaseInvoice)
    invoice_deleted = Signal(int)

    def __init__(self, 
                 purchase_invoice_controller: PurchaseInvoiceController | None = None,
                 party_controller: PartyController | None = None,
                 item_controller: ItemController | None = None,
                 parent=None):
        super().__init__(parent)
        self.invoice_controller = purchase_invoice_controller or PurchaseInvoiceController()
        ...
        self._all_suppliers = []  # Store all suppliers for filtering
    ...
    def _build_ui(self) -> None:
        ...
        self.search_input = QLineEdit()
        self.search_input.setPlaceholderText("Search by invoice number or supplier...")
        ...
        form_group = QGroupBox("Purchase Invoice Details")
        form_layout = QFormLayout(form_group)

        self.invoice_number_input = QLineEdit()
        self.invoice_number_input.setPlaceholderText("e.g., PI-2026-001")
        ...
        # Removed auto-save on supplier search to allow adding items first
        supplier_layout.addWidget(self.supplier_search)
        
        self.supplier_input = QComboBox()
        self.supplier_input.addItem("Select Supplier", None)
        # Removed auto-save on supplier selection to allow adding items first
        ...
        self.items_table.setHorizontalHeaderLabels([
            "Item", "Quantity", "Unit Cost", "Discount", "Tax", "Line Total"
        ])
        ...
        self.pay_button = QPushButton("Pay Supplier")
        self.pay_button.clicked.connect(self._on_pay_supplier)
        self.pay_button.setEnabled(False)
        self.pay_button.setStyleSheet("background: #2ecc71; color: white; font-weight: bold;")
    ...
    def _populate_invoice_table(self):
        ...
        self.table.setHorizontalHeaderLabels([
            "Invoice #", "Supplier", "Date", "Total", "Paid", "Status", "Actions"
        ])
        
        for row, invoice in enumerate(invoices):
            self.table.setItem(row, 0, QTableWidgetItem(invoice.invoice_number))
            supplier_name = "Unknown"
            for p in self._suppliers_cache:
                if p.id == invoice.supplier_id:
    ...
    def _load_suppliers_async(self):
        """Load suppliers asynchronously using background thread."""
        if self._supplier_load_thread and self._supplier_load_thread.isRunning():
            self._supplier_load_thread.terminate()
    ...
    def _populate_supplier_dropdown(self):
        ...
        self.supplier_input.addItem("Select Supplier", None)
        for party in self._suppliers_cache:
            if party.party_type in ["SUPPLIER", "BOTH"]:
                display = f"{party.name} ({party.code})"
                self.supplier_input.addItem(display, party.id)

    def _filter_suppliers(self, text: str) -> None:
        text = text.lower().strip()
        current_data = self.supplier_input.currentData()
        
        self.supplier_input.clear()
        self.supplier_input.addItem("Select Supplier", None)
        
        for party in self._all_suppliers:
            if party.party_type in ["SUPPLIER", "BOTH"]:
        ...
        if current_data:
            idx = self.supplier_input.findData(current_data)
            if idx >= 0:
    ...
    def _on_table_clicked(self, index) -> None:
        ...
        if invoice:
            ...
            idx = self.supplier_input.findData(invoice.supplier_id)
            if idx >= 0:
                self.supplier_input.setCurrentIndex(idx)
            ...
            self.pay_button.setEnabled(is_credit and has_balance and is_active)
            
            if not is_credit:
                self.pay_button.setToolTip("Only credit invoices can be paid")
            elif not has_balance:
                self.pay_button.setToolTip("This invoice is already fully paid")
            elif not is_active:
                self.pay_button.setToolTip("Cannot pay a cancelled invoice")
            else:
                self.pay_button.setToolTip(f"Pay remaining: Rs. {remaining:,.2f}")
    ...
    def _load_invoice_items(self, invoice_id: int) -> None:
        ...
        for row, item in enumerate(items):
            ...
            self.items_table.setItem(row, 2, QTableWidgetItem(f"{item.get('unit_cost', 0):.2f}"))
    ...
    def _on_add_item_clicked(self) -> None:
        dialog = PurchaseItemSelectionDialog(self.item_controller, self)
    ...
    def _on_save_clicked(self):
        ...
        if not invoice_number:
            ...
            invoice_number = journal_repo.next_voucher_number(1, "PURCHASE_INVOICE")
        ...
        for row in range(self.items_table.rowCount()):
            ...
            unit_cost_item = self.items_table.item(row, 2)
            discount_item = self.items_table.item(row, 3)
            tax_item = self.items_table.item(row, 4)
            
            if not all([item_name_item, quantity_item, unit_cost_item, discount_item, tax_item]):
                continue
            ...
            try:
                ...
                items.append({
                    ...
                    "unit_cost": float(unit_cost_item.text()),
    ...
    def _on_delete_clicked(self) -> None:
        ...
        if reply == QMessageBox.Yes:
            success, error = self.invoice_controller.delete_purchase_invoice(self._selected_invoice_id)
            if success:
    ...
    def _on_pay_supplier(self):
        if not self._selected_invoice_id:
            QMessageBox.warning(self, "Selection Error", "Please select an invoice to pay.")
            return
        ...
        if invoice.status == "CANCELLED":
            QMessageBox.warning(self, "Error", "Cannot pay a cancelled invoice.")
            return
        
        if invoice.payment_type != "CREDIT":
            QMessageBox.warning(self, "Error", "Only credit invoices can be paid.")
            return
        
        remaining = invoice.total_amount - invoice.paid_amount
        supplier_name = self._get_supplier_name(invoice.supplier_id)
        
        dialog = QDialog(self)
        dialog.setWindowTitle(f"Pay Supplier - {invoice.invoice_number}")
        ...
        if dialog.exec() == QDialog.Accepted:
            ...
            success, error = self.payment_controller.pay_supplier(
            ...
            if success:
                ...
                QMessageBox.information(self, "Success", f"Payment of Rs. {amount:,.2f} recorded successfully!")
                self._load_invoices()
                self._clear_form()
            else:
