...
class PurchaseInvoiceRepository(BaseRepository[PurchaseInvoice]):
    """Repository for PurchaseInvoice operations"""
    
    def __init__(self):
        super().__init__(PurchaseInvoice, 'purchase_invoices')
        self.lines_repo = PurchaseInvoiceLineRepository()
    
    def get_by_invoice_number(self, invoice_number: str, company_id: int) -> PurchaseInvoice | None:
    ...
    def get_by_company(self, company_id: int) -> list[PurchaseInvoice]:
        """Get all invoices for a company"""
        return self.get_all("company_id = ?", (company_id,), "date DESC")
    
    def get_by_party(self, party_id: int, company_id: int) -> list[PurchaseInvoice]:
    ...
    def get_unpaid_invoices(self, company_id: int) -> list[PurchaseInvoice]:
    ...
    def get_by_date_range(self, company_id: int, start_date: str, end_date: str) -> list[PurchaseInvoice]:
    ...
    def create_with_lines(self, invoice: PurchaseInvoice) -> int:
        """Create purchase invoice with its lines in a transaction"""
        with db.transaction() as cursor:
            ...
            cursor.execute(
                f"INSERT INTO purchase_invoices ({columns_str}) VALUES ({placeholders})",
                tuple(values)
            )
            ...
            for line in invoice.lines:
                line.purchase_invoice_id = invoice_id
...
class PurchaseInvoiceLineRepository:
    """Repository for PurchaseInvoiceLine operations"""
    
    def create_with_cursor(self, cursor, line: PurchaseInvoiceLine) -> int:
        """Create a purchase invoice line using provided cursor"""
        cursor.execute(
            """INSERT INTO purchase_invoice_lines 
               (purchase_invoice_id, item_id, item_name, item_code,
                quantity, rate, amount, discount_percent, discount_amount,
                tax_rate, tax_amount, net_amount, batch_number,
                warehouse_id, warehouse_name, manufacturing_date, expiry_date, narration)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            (
    ...
    def get_by_invoice(self, purchase_invoice_id: int) -> list[PurchaseInvoiceLine]:
        """Get all lines for an invoice"""
        rows = db.fetch_all(
            "SELECT * FROM purchase_invoice_lines WHERE purchase_invoice_id = ? ORDER BY id",
            (purchase_invoice_id,)
        )
        return [PurchaseInvoiceLine.from_row(row) for row in rows]
